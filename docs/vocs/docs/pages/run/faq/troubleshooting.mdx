...
2. Have slow canonical commit time according to the `Canonical Commit Latency Time` chart on [Grafana dashboard]([Grafana dashboard]([Grafana dashboard](/run/monitoring#observability-with-prometheus-grafana) (more than 2-3 seconds)

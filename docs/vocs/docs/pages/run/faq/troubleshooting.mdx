...
2. Have slow canonical commit time according to the `Canonical Commit Latency Time` chart on [Grafana dashboard](/run/monitoring.mdx) (more than 2-3 seconds)
...
To confirm it, check if the values on the `Freelist` chart on [Grafana dashboard](/run/monitoring.mdx)
is greater than 10M.
